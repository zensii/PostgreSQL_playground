...
AND id NOT IN (
    SELECT courses.client_id FROM courses
    );
...
FROM cars c LEFT JOIN courses c2 ON c.id = c2.car_id
GROUP BY car_id, make, mileage, c.id
...
$$
    ...
    BEGIN
        ...
        SELECT
            count(c2.id) INTO result
        FROM
        clients c LEFT JOIN courses c2 ON c.id = c2.client_id
...
$$
    BEGIN
        ...
        FROM
            addresses AS ad
                JOIN courses co ON ad.id = co.from_address_id
